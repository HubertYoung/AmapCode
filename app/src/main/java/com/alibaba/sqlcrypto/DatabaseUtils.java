...
import com.alibaba.sqlcrypto.sqlite.SQLiteProgram;
...
public class DatabaseUtils {
    ...
    public class InsertHelper {
        ...
        private void buildSQL() {
            ...
            Cursor cur = null;
            try {
                Cursor cur2 = this.mDb.rawQuery("PRAGMA table_info(" + this.mTableName + ")", null);
                ...
                while (cur2.moveToNext()) {
                    ...
                    sb.append(i == cur2.getCount() ? ") " : ", ");
                    sbv.append(i == cur2.getCount() ? ");" : ", ");
                    i++;
                }
                if (cur2 != null) {
                    cur2.close();
    ...
    public static String getHexCollationKey(String name) {
        byte[] arr = getCollationKeyInBytes(name);
        return new String(Hex.encodeHex(arr), 0, getKeyLen(arr) * 2);
    }
    ...
    public static boolean queryIsEmpty(SQLiteDatabase db, String table) {
        return longForQuery(db, new StringBuilder("select exists(select 1 from ").append(table).append(")").toString(), null) == 0;
    }
