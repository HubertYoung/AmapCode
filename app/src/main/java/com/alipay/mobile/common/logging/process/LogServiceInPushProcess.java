...
public class LogServiceInPushProcess extends IntentService {
            ...
            if (!TextUtils.isEmpty(action) && extras != null) {
                ...
                if (action.equals(getPackageName() + ".monitor.action.upload.mdaplog")) {
                    try {
                        ...
                        LoggerFactory.getTraceLogger().error((String) "LogServiceInPushProcess", "ACTION_UPLOAD_MDAPLOG: " + t.toString());
                    }
                    ...
                    LoggerFactory.getTraceLogger().info("LogServiceInPushProcess", action + ", type: " + type);
                    a(type, value);
                } else if (action.equals(getPackageName() + ".monitor.action.UPDATE_LOG_CONTEXT_BATCH")) {
                    LoggerFactory.getTraceLogger().info("LogServiceInPushProcess", action + ", size: " + extras.size());
                    ...
                    LoggerFactory.getTraceLogger().error((String) "LogServiceInPushProcess", "no such action: " + action);
                }
            }
        }
    }
    ...
    private static void a(String type, String value) {
        if (LoggingSPCache.STORAGE_CHANNELID.equals(type)) {
            ...
            LoggerFactory.getTraceLogger().error((String) "LogServiceInPushProcess", "not mapping, type: " + type + ", value: " + value);
        }
    }
}
