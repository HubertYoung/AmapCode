...
public class H5PagePreloadFactoryImpl implements H5PageFactory {
    ...
    public H5ViewHolder createPage(Activity activity, Bundle param) {
        H5Log.d(TAG, "start create page");
        long time = System.currentTimeMillis();
        H5ViewHolder rootViewHolder = new H5ViewHolder();
        H5PageImpl h5Page = new H5PageImpl(activity, rootViewHolder);
    ...
    public void setUpPage(H5ViewHolder rootViewHolder, final Bundle startParams) {
        final H5PageImpl h5Page = rootViewHolder.getH5Page();
        boolean transActivity = H5Utils.getBoolean(startParams, (String) H5Param.LONG_TRANSPARENT, false);
        H5Log.d(TAG, "transActivity " + transActivity);
        if (!transActivity) {
            H5Log.d(TAG, "init nav bar");
            try {
                rootViewHolder.setH5NavBar(new H5NavigationBar(this.a, startParams, rootViewHolder));
            } catch (Throwable t) {
        ...
        new KeyboardUtil(this.a).setListener(new KeyboardListener() {
            public void onKeyboardVisible(boolean visible) {
                H5Log.d(H5PagePreloadFactoryImpl.TAG, "onKeyboardVisible " + visible);
                if (visible) {
