...
public class H5PaintImageFilterPlugin extends MMH5SimplePlugin {
    public static final String ACTIONS = "paintImage";

    public H5PaintImageFilterPlugin() {
    ...
    public boolean handleEvent(H5Event event, final H5BridgeContext context) {
        Logger.debug("H5PaintImageFilter", "handleEvent event: " + event + ", context: " + context);
        ...
        if (TextUtils.isEmpty(data)) {
            ...
            public void run() {
                try {
                    Bitmap bitmap = H5PaintImageFilterPlugin.b(data);
                    if (bitmap != null) {
                        String base64Data = Base64Utils.encodeToBase64(((MultimediaImageProcessor) Utils.getService(MultimediaImageProcessor.class)).pictureOilFilter(bitmap));
                        if (!TextUtils.isEmpty(base64Data)) {
                            H5PaintImageFilterPlugin.b(context, base64Data);
                        } else {
                            H5PaintImageFilterPlugin.b(context, -3);
                        }
                    } else {
                        H5PaintImageFilterPlugin.b(context, -1);
                    }
                } catch (Exception e) {
                    Logger.error("H5PaintImageFilter", "handleEvent error", e);
                    H5PaintImageFilterPlugin.b(context, -2);
                }
            }
        })) {
    ...
    /* access modifiers changed from: private */
    public static void b(H5BridgeContext context, String base64Data) {
        JSONObject result = new JSONObject();
        result.put((String) "status", (Object) Integer.valueOf(0));
        result.put((String) "msg", (Object) a(0));
        result.put((String) "image", (Object) base64Data);
