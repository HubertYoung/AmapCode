...
public class FreerideTipView implements OnLayoutChangeListener, OnGlobalLayoutListener, IViewLayer, IViewLayerExt {
    private static final String FREE_RIDE_TIPS_BOTTOM_POSITION = "bottom";
    private static final String FREE_RIDE_TIPS_MIDDLE_POSITION = "middle";
    private static final String FREE_RIDE_TIPS_TOP_POSITION = "top";
    public static final String SP_KEY_DIY_FREE_RIDE_ENTRY_SHOW = "diy_free_ride_entry_show";
    private static final String TAG = "FreeRideTipView";
    ...
    class AutoDismissTipsRunnable implements Runnable {
        ...
        public void run() {
            if (FreerideTipView.this.mPageContext.isAlive()) {
                FreerideTipView.this.dismiss();
            }
        }
    }
    ...
    public FreerideTipView(bid bid) {
    ...
    public void show(ViewGroup viewGroup, View view, List<DIYMainMapEntry> list) {
        if (!isShowing()) {
            ...
            if (findEntryAnchorView != null) {
                ...
                new MapSharePreference(SharePreferenceName.SharedPreferences).putBooleanValue(SP_KEY_DIY_FREE_RIDE_ENTRY_SHOW, false);
    ...
    private void getAnchorViewPosition(ViewGroup viewGroup, List<DIYMainMapEntry> list) {
        View findViewWithTag = viewGroup.findViewWithTag(DIYMainMapPresenter.DIY_ENTRY_KEY_FREERIDE);
        if (findViewWithTag == null && list.size() > DIYMainMapPresenter.MAX_ENTRY_NUMBER) {
            this.tipsPosition = FREE_RIDE_TIPS_BOTTOM_POSITION;
        } else if (findViewWithTag == null || !DIYMainMapPresenter.DIY_ENTRY_KEY_FREERIDE.equals(list.get(0).key)) {
            this.tipsPosition = FREE_RIDE_TIPS_MIDDLE_POSITION;
        } else {
    ...
    private void getAnchorLocation(View view) {
        ...
        if (view != null) {
            ...
            if (this.tipsPosition.equals(FREE_RIDE_TIPS_MIDDLE_POSITION)) {
                iArr[1] = iArr[1] - ((view.getHeight() * 4) / 5);
            } else if (this.tipsPosition.equals("top")) {
                iArr[1] = iArr[1] - ((view.getHeight() * 3) / 5);
            } else if (this.tipsPosition.equals(FREE_RIDE_TIPS_BOTTOM_POSITION)) {
    ...
    private View findEntryAnchorView(ViewGroup viewGroup, View view, List<DIYMainMapEntry> list) {
        ...
        while (true) {
            ...
            if (next != null && TextUtils.equals(next.key, DIYMainMapPresenter.DIY_ENTRY_KEY_FREERIDE)) {
