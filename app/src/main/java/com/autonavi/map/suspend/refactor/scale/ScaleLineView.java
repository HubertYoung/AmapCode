...
public class ScaleLineView extends View {
    ...
    public float cur_level = 0.0f;
    private Bitmap logoBitmap = null;
    public boolean mAlignRight = true;
    Context mContext;
    private MapManager mMapManager;
    private float mTextSize = ((float) agn.a(getContext(), 14.0f));
    ...
    public void onDraw(Canvas canvas) {
        if (this.mMapManager != null) {
            if (this.mMapManager.getMapMode() == 2) {
    ...
    public void paintScaleLine(Canvas canvas) {
        ...
        this.cur_level = this.mMapManager.getMapView().v();
        ...
        int width = getWidth();
        int i2 = (int) (this.cur_level - 1.0f);
        this.centerX = this.mMapManager.getMapView().p();
        this.centerY = this.mMapManager.getMapView().q();
        ...
        paint.setTextSize(this.mTextSize == 0.0f ? (float) agn.a(getContext(), 14.0f) : this.mTextSize);
    ...
    public void refreshScaleLineView() {
        if (this.mMapManager != null) {
            bty mapView = this.mMapManager.getMapView();
            if (mapView != null && Math.abs(this.cur_level - mapView.v()) > 1.0E-7f) {
                postRefresh();
            }
        }
    }
